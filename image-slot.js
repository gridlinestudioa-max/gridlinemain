...
(() => {
  ...
  // Full-bleed hero panels can render ~830 CSS px wide and taller still when
  // a tall image fills an expanded panel; at 2× device pixels that wants
  // ~2400px+. Cap generously so cover-scaled cards stay retina-sharp.
  const MAX_DIM = 1600;
  ...
  // ── SVG auto-trim ───────────────────────────────────────────────────────
  // Exports often park artwork in a corner of a large artboard, or carry an
  // invisible full-canvas element, so background-size:contain fits empty space
  // and the logo looks tiny / off-center. Rasterize the SVG and scan for the
  // visible (non-transparent) pixel bounds, then rewrite the viewBox to that
  // region so the mark fills and centers. getBBox is unreliable here (it counts
  // invisible geometry), so we measure actual pixels. Async (needs an image
  // decode); memoized by URL, re-renders the slot when the trim is ready.
  const _svgTrimCache = new Map(); // url -> trimmed url (or original on failure)
  const _svgTrimPending = new Set();
  ...
  async function _pixelTrimSvg(url) {
    ...
    if (!svg || svg.tagName.toLowerCase() !== 'svg') return url;
    const vb = (svg.getAttribute('viewBox') || '').trim().split(/[\s,]+/).map(Number);
    let vx, vy, vw, vh;
    if (vb.length === 4 && vb.every(Number.isFinite)) { [vx, vy, vw, vh] = vb; }
    else { vx = 0; vy = 0; vw = parseFloat(svg.getAttribute('width')) || 300; vh = parseFloat(svg.getAttribute('height')) || 150; }
    const maxDim = 1000;
    ...
    if (maxX < 0) return url; // fully transparent — leave alone
    const ux = vx + minX / scale, uy = vy + minY / scale;
    const uw = (maxX - minX + 1) / scale, uh = (maxY - minY + 1) / scale;
    const pad = Math.max(uw, uh) * 0.05;
    svg.setAttribute('viewBox', (ux - pad) + ' ' + (uy - pad) + ' ' + (uw + pad * 2) + ' ' + (uh + pad * 2));
    svg.removeAttribute('width');
    svg.removeAttribute('height');
    const str = new XMLSerializer().serializeToString(svg);
    return 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(str)));
  }
  function tightenSvg(url, onReady) {
    if (_svgTrimCache.has(url)) return _svgTrimCache.get(url);
    if (!_svgTrimPending.has(url)) {
      _svgTrimPending.add(url);
      _pixelTrimSvg(url).then((res) => {
        _svgTrimCache.set(url, res || url); _svgTrimPending.delete(url); onReady && onReady();
      }).catch(() => {
        _svgTrimCache.set(url, url); _svgTrimPending.delete(url); onReady && onReady();
      });
    }
    return url; // original shown until the trim resolves
  }
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      // SVG is vector — never rasterize it through the canvas (that would
      // downscale it to MAX_DIM and destroy the crispness). Store the file
      // bytes as a data URL untouched so it stays infinitely sharp.
      if (file.type === 'image/svg+xml') {
        ...
        try {
          ...
          console.warn('<image-slot> svg ingest failed:', err);
        }
        return;
      }
    ...
    // pan toward zero.
    _geom() {
      // SVG sources report unreliable/absent intrinsic dimensions, so the
      // %-based fit math mis-scales them. Return null to fall back to CSS
      // object-fit, which honors contain/cover against the frame directly.
      if (this._img && /^data:image\/svg\+xml/i.test(this._img.src || '')) return null;
    ...
    _render() {
      ...
      if (url && !attrError) {
        ...
        if (isSvg) {
          // Render SVG as a frame background: background-size honors
          // contain/cover deterministically and stays centered even when the
          // SVG carries no intrinsic width/height (an <img> defaults such an
          // SVG to 300x150 — small and pinned top-left).
          this._img.style.display = 'none';
          this._ghost.removeAttribute('src');
          this._frame.style.backgroundImage = 'url("' + tightenSvg(url, () => this._render()) + '")';
          this._frame.style.backgroundRepeat = 'no-repeat';
          this._frame.style.backgroundPosition = 'center';
          this._frame.style.backgroundSize =
            (this.getAttribute('fit') || 'cover').toLowerCase() === 'contain' ? 'contain' : 'cover';
